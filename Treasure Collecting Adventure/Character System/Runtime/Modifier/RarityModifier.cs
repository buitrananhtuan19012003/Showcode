...
{
    [CreateAssetMenu(fileName = "SimpleRarityModifier", menuName = "Treasure Collecting Adventure/Character System/Modifiers/Rarity")]
    [System.Serializable]
    public class RarityModifier : CharacterModifier
    {
        [SerializeField]
        ...
        {
            player.Rarity = SelectRarity(this.m_Rarities);
            ApplyPropertyMultiplier(player, player.Rarity.Multiplier);
        }

        protected virtual void ApplyPropertyMultiplier(Player player, float multiplier)
        {
            ObjectProperty[] properties = player.GetProperties();
