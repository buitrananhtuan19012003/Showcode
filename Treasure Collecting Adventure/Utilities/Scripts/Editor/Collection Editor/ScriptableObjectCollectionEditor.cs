...
namespace LupinrangerPatranger
{
    ...
    public class ScriptableObjectCollectionEditor<T> : CollectionEditor<T> where T : ScriptableObject, INameable
    {
        ...
        //protected override List<T> Players
        //{
        //    get { return players; }
        //}
        ...
        protected virtual void CreatePlayer(Type type)
        {
            T player = (T)ScriptableObject.CreateInstance(type);
            player.hideFlags = HideFlags.HideInHierarchy;
            AssetDatabase.AddObjectToAsset(player, target);
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            //Players.Add(player);
            Select(player);
