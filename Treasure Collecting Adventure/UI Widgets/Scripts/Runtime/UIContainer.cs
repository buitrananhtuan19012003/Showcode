...
namespace LupinrangerPatranger.UIWidgets
{
    public class UIContainer<T> : UIWidget where T : class
    {
        ...
        public virtual bool AddCharacter(T player)
        {
            UISlot<T> slot = null;
        ...
        public virtual bool RemoveCharacter(int index)
        {
            ...
            {
                UISlot<T> slot = this.m_Slots[index];
                T player = slot.ObservedCharacter;
        ...
        public virtual T ReplaceCharacter(int index, T character)
        {

            if (index < this.m_Slots.Count)
            {
                UISlot<T> slot = this.m_Slots[index];
                if (!slot.CanAddCharacter(character))
                {
                    return character;
                }
                ...
                if (character != null)
                {
                    this.m_Collection.Add(character);

                    T current = slot.ObservedCharacter;
                    if (current != null)
                    {
                        RemoveCharacter(slot.Index);
                    }
        ...
        public virtual bool CanAddCharacter(T player, out UISlot<T> slot, bool createSlot = false)
        {
            slot = null;
            ...
            for (int i = 0; i < this.m_Slots.Count; i++)
            {
                if (this.m_Slots[i].IsEmpty && this.m_Slots[i].CanAddItem(player))
                {
                    slot = this.m_Slots[i];
                    return true;
                }
